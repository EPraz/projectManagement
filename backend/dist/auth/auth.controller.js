...
let AuthController = class AuthController {
    ...
    async register(res, request) {
        const { accessToken, refreshToken } = await this.authService.register(request);
    ...
    async instantLogin(res, email) {
        if (!email || !email.includes('@')) {
            throw new common_1.BadRequestException('Invalid email format');
        }
        const { accessToken, refreshToken } = await this.authService.instantLogin(email);
        res.cookie('jwt', refreshToken, {
            httpOnly: true,
            secure: process.env.NODE_ENV === 'production',
            sameSite: 'lax',
...
__decorate([
    ...
    __metadata("design:paramtypes", [Object, dto_1.RegisterDto]),
...
__decorate([
    (0, common_1.Post)('instant-login'),
    __param(0, (0, common_1.Res)({ passthrough: true })),
    __param(1, (0, common_1.Body)('email')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, String]),
    __metadata("design:returntype", Promise)
], AuthController.prototype, "instantLogin", null);
